...
module Align
  ...
  class Blosum62 < BasicScoring
    ...
    def initialize(align_match=nil, align_mismatch=nil, gap_penalty)
      super(align_match, align_match, gap_penalty)

      @blosum62 = read_in_scoring_matrix()
    end
    ...
    def read_in_scoring_matrix
      ...
      blosum62_declaration.each_line do |line|
        line = line.strip
        if aas.nil?
          ...
          aas.each_with_index do |aa2, ind|
            mat[aa1.to_s.upcase + aa2.to_s.upcase] = cells[ind]
          end
        end
      end
    ...
    def score_align(a,b)
      key = (a.chr + b.chr).upcase
      if ! @blosum62.has_key?(key) then 
        key = "XX"
      end
      @blosum62[ key ].to_i
    end
    ...
    def blosum62_declaration
      blosum62_declaration = <<-EOS
